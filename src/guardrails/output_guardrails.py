...
from core.config import LITELLM_MODEL
...
def content_filter(response: str) -> dict:
    ...
    # PII / secret patterns to check. Order matters: redact specific secrets
    # (API key, password, db host) before the broader number patterns.
    PII_PATTERNS = {
        "API key": r"sk-[a-zA-Z0-9-]+",
        "Password": r"password\s*(is|[:=])\s*\S+",
        "DB connection": r"[\w.-]+\.internal(:\d+)?",
...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    async def after_model_callback(
        ...
        *,
        ...
        # Layer 1: deterministic content filter — redact PII/secrets.
        ...
        # Layer 2: LLM-as-Judge — semantic safety check on the (redacted) text.
